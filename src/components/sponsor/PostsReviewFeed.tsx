...
export function PostsReviewFeed() {
  ...
  async function handleApprove(postId: string) {
    setError(null);
    setActioning(postId);
    const result = await (supabase as any).rpc("approve_post", {
      post_id: postId,
    });
    if (result.error) {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="heading-page mb-6">Review Posts</h1>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-semibold text-gray-900">
                      {post.athlete_name ?? "Unknown athlete"}
                    </span>
                  ...
                  {post.link_url && (
                    <a
                      ...
                      className="text-xs text-brand-blue hover:underline mt-1 inline-block truncate max-w-sm"
                    >
                      {post.link_url}
                    </a>
                  )}
