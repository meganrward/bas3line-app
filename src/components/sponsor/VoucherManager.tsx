...
export function VoucherManager() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="heading-page mb-6">Vouchers</h1>

      <form onSubmit={handleAdd} className="card p-5 mb-6 space-y-3">
        <h2 className="heading-section">Add voucher code</h2>
      ...
      <div className="space-y-6">
        <div>
          <h2 className="heading-section mb-3">
            Available ({unusedVouchers.length})
          </h2>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {voucher.description ?? "—"} · {voucher.points_required}{" "}
                      pts required
                    </p>
                  </div>
                  <button
          ...
          <div>
            ...
            <ul className="card divide-y divide-gray-100">
              {usedVouchers.map((voucher) => (
                ...
                >
                  <div>
                    <p className="font-mono text-sm font-semibold text-gray-500 line-through">
                      {voucher.code}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {voucher.description ?? "—"} · redeemed by{" "}
                      {voucher.used_by_name ?? "unknown"}
                      {voucher.used_at
                        ? ` on ${new Date(voucher.used_at).toLocaleDateString()}`
                        : ""}
                    </p>
                  </div>
                </li>
